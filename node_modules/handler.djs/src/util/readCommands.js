...
module.exports = async function(pathManeger) {
  ...
  for await (const file of files) {
    ...
    if (commands.find(e => e.name.toLowerCase() === cmd.name.toLowerCase()  && cmd.disabed === false)) throw new DiscordjsTypeError(ErrorCodes.InvalidType, `${file}`, "repeated command", true) 
